...
import { createImgElement } from "./dom-fragment-utilities.js";
...
class ContentPanel {
  constructor(projectList, docObj, navPanel) {
    ...
    this.editProjImg.addEventListener("click", () => {
      if (!this.projectDialog) {
        this.projectDialog = new EditProjectDialog(
          this.docObj,
          this.projectList,
          this,
          this.navPanel
  ...
  showTaskDisabled(taskId, taskRootEl, flag) {
    const allTaskElements = this.docObj.querySelectorAll(
      `[data-id="${taskId}"]`
    );
    if (flag) {
  ...
  taskClicked = (e) => {
    ...
    const proj = this.projectList.getProj(
      taskCurrentProjectId ?? e.target.getAttribute("data-proj")
    );
    ...
    if (e.target.classList.contains("tooltiptext")) {
      ...
      const taskId =
        ...
        e.target.parentElement.getAttribute("data=id");
      const projId =
        e.target.getAttribute("data-proj") ??
        e.target.parentElement.getAttribute("data-proj");
      if (taskId && projId) {
        this.editTaskDialog.show(
          this.projectList.getProj(projId).getTask(taskId),
          this.projectList.getProj(projId)
        );
        this.projectList.updateStorage();
      }
  ...
  displayTasksHelper(task, projId, isInViewPane) {
    ...
    divEl.style.background = isInViewPane
      ? `linear-gradient(20deg, transparent, transparent 95%, ${projObj.color} 95%, ${projObj.color} 100%)`
      : "transparent";
    ...
    const priority = priorityStrings[task.priority];
  ...
  displayProject = (id) => {
    ...
    if (id != 0) {
      // this is not the default project so display delete image
