...
class TaskDialog {
  constructor(docObj, projectList, navPanel, contentPanel, parentSelector) {
    ...
    this.taskDialogDiv = docObj.querySelector(
      `${parentSelector}>form>div:first-child`
    );
    this.projectListEl = docObj.querySelector(
      `${parentSelector} #project-list`
    );
  ...
  showDialog() {
    ...
    for (const id in projects) {
      const projEl = this.navPanel.createNameEl(
        projects[id],
        "option",
        currentProject === id
      );
...
class EditTaskDialog extends TaskDialog {
  constructor(docObj, projectList, navPanel, contentPanel) {
    ...
    const editBtn = docObj.querySelector(
      `${this.parentSelector} .button-panel>.save`
    );
  ...
  loadFields(taskObj, projObj) {
    //loads the current task's values into the fields
  ...
  editTask(e) {
    const taskIdEl = this.docObj
      .querySelector(`${this.parentSelector} #hidden-span`)
      .getAttribute("data-id");
    const taskProjIdEl = this.docObj
      .querySelector(`${this.parentSelector} #hidden-span`)
      .getAttribute("data-proj");
    const nameEl = this.docObj.querySelector(
      `${this.parentSelector} #task-name`
    );
    const color = this.docObj.querySelector(
      `${this.parentSelector} #task-color`
    ).value;
    const priority = this.docObj.querySelector(
      `${this.parentSelector} #task-priority`
    ).value;
    const desc = this.docObj.querySelector(
      `${this.parentSelector} #task-desc`
    ).value;

    const dueDate = this.dueDateEl.value
      ? makeUTCString(this.dueDateEl.value)
      : "";
    const project = this.docObj.querySelector(
      `${this.parentSelector} #project-list`
    ).value;
    ...
    if (validityState.valid) {
      ...
      const newTask =
        priority === ""
          ? createTask(nameEl.value, color, desc, dueDate, false, taskIdEl)
          : createTask(
              nameEl.value,
              color,
              desc,
...
class AddTaskDialog extends TaskDialog {
  constructor(docObj, projectList, navPanel, contentPanel) {
    ...
    const addBtn = docObj.querySelector(
      `${this.parentSelector} .button-panel>.add`
    );
  ...
  addTask(e) {
    ...
    if (validityState.valid) {
      const newTask =
        priority === ""
          ? createTask(nameEl.value, color, desc, dueDate)
